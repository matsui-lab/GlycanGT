...
RUN_NAME = "node70_edge0"
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'outputs', RUN_NAME)
...
def save_summary_results(results: dict):
    ...
    output_path = os.path.join(OUTPUT_DIR, f'training_summary_{RUN_NAME}.txt')
    with open(output_path, 'w') as f:
        f.write(f"--- TokenGT Multi-Task Pre-training Summary ({RUN_NAME}) ---\n\n")
...
def train_and_validate(config_name: str, vocabs: tuple, all_file_paths: list, hparams: dict, MASK_IDS: dict):
    ...
    for epoch in range(hparams['num_epochs']):
        ...
        node_ratio = hparams.get('node_mask_ratio', 0.35)
        edge_ratio = hparams.get('edge_mask_ratio', 0.35)
        train_progress_bar = tqdm(
            train_loader,
            desc=f"[{RUN_NAME}] Epoch {epoch+1} Train (node {node_ratio:.2f}, edge {edge_ratio:.2f})"
            )
        ...
        for batch in train_progress_bar:
            ...
            node_mask = torch.rand_like(original_node_data.float()) < node_ratio
            edge_mask = torch.rand_like(original_edge_data.float()) < edge_ratio
...
if __name__ == "__main__":
    ...
    'learning_rate': 1e-6,
    'batch_size': 32,
    'node_mask_ratio': 0.70,
    'edge_mask_ratio': 0.00,
    ...
    for size in model_sizes_to_run:
        ...
        history_filename = os.path.join(OUTPUT_DIR, f"loss_history_{size}_multitask_{RUN_NAME}.csv")
