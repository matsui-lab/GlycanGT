    ...
    def forward(
        ...
        * NTP  expects `input_ids` to include BOS, we predict next
        ...
        if attn_dict is not None:
            maps = attn_dict.get("maps", None)
            if isinstance(maps, dict) and len(maps) > 0:
                try:
                    # maps: {layer_idx: [B, H, L, L]}
                    keys_sorted = sorted(maps.keys())
                    attn_all = [maps[k] for k in keys_sorted]
                    attn_last = attn_all[-1]
                except Exception:
                    attn_all = None
                    attn_last = None
            if attn_last is None:
                attn_last = attn_dict.get("maps_last") or attn_dict.get("last")
