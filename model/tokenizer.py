"""
Modified from https://github.com/jw9730/tokengt/blob/main/large-scale-regression/tokengt/modules/tokenizer.py
"""
# tokenizer.py
import math
...
class GraphFeatureTokenizer(nn.Module):
    ...
    def get_index_embed(node_id, node_mask, padded_index):
        """
        :param node_id:     Tensor([sum(node_num), D])  ← バッチ内全ノードの連結テンソル
        :param node_mask:   BoolTensor([B, max_n])      ← 各グラフの有効ノード位置
        :param padded_index:LongTensor([B, T, 2])       ← 各トークンが参照する(ローカル)ノードindex×2
        ...
        padded_node_id[node_mask] = node_id                                                     # ローカルindex領域に展開
        
        # [B, max_n, 2, D] と [B, T, 2, D] に合わせて gather
    ...
    def forward(self, batched_data, perturb=None):
        ...
        if self.rand_node_id:
            ...
            rand_index_embed = self.get_index_embed(rand_node_id, node_mask, padded_index)
