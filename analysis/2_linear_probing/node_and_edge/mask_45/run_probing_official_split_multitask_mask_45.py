...
PRETRAINED_DIR = os.path.join(PROJECT_ROOT, 'outputs', 'mask_45')
FEATURE_CACHE_DIR = os.path.join(PROJECT_ROOT, 'outputs', 'feature_cache_mask_45')
...
def load_pretrained_multitask_model(size, vocabs, pretrained_dir, device):
    ...
    model_path = os.path.join(pretrained_dir, f"pretrained_tokengt_{size}_multitask_mask_45_final.pt")
...
def main():
    print("====== Linear Probing with Official Splits for mask 45 Multi-task Model ======")
    ...
    for size in model_sizes:
        ...
        for task_name, params in TASKS.items():
            ...
            feature_cache_path = os.path.join(FEATURE_CACHE_DIR, f"features_{size}_multitask_mask_45_{params['graph_subdir']}.pt")
            ...
            for target_col in target_cols:
                ...
                for clf_type in classifiers_to_run:
                    ...
                    for seed in seeds_to_run:
                        ...
                        all_params.append({
                            'Model Size': f"{size}_multitask_mask_45",
                            'Task': target_col,
    ...
    output_path = os.path.join(PROJECT_ROOT, 'analysis', '2_linear_probing', 'node_and_edge', 'mask_45', 'linear_probing_official_split_multitask_mask45_hpo_svm_lgbm_results.csv')
    
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    results_df.to_csv(output_path, index=False)
    
    params_df = pd.DataFrame(all_params)
    params_output_path = os.path.join(PROJECT_ROOT, 'analysis', '2_linear_probing', 'node_and_edge', 'mask_45',
                                      'linear_probing_official_split_multitask_mask45_hpo_best_params.csv')
